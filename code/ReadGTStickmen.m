function [lF] = ReadGTStickmen(txtfile,dsid,vid)
...
while ~stop,
    ...
    if count < 1
        ...
        for itrGT = 1:numGT
            ...
            % Processing Ground truth for Invisible coordinates and parts
            negFlags = sum((gtsticks.coor' < 0),2) > 0;
            
            %All the parts whose coorinates are not visible will be skipped. Thus only fully visible and frontal poses are considered
            if sum(negFlags) > 0
                continue;
            end
